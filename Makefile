...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app ibom_ai_agent_stack db upgrade
	@echo ""
...
seed:
	...
	@if docker compose -f docker-compose.dev.yml exec -T app \
		ibom_ai_agent_stack user list 2>/dev/null \
		...
		docker compose -f docker-compose.dev.yml exec -T app \
			ibom_ai_agent_stack user create \
...
stage:
	...
	docker compose -f docker-compose.yml exec -T app ibom_ai_agent_stack db upgrade
...
prod:
	...
	@sleep 5
	docker compose --env-file backend/.env -f docker-compose.prod.yml exec -T app ibom_ai_agent_stack db upgrade
...
db-init: docker-db
	...
	@sleep 8
	cd backend && uv run ibom_ai_agent_stack db migrate -m "initial" || true
	cd backend && uv run ibom_ai_agent_stack db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend ibom_ai_agent_stack db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend ibom_ai_agent_stack db upgrade

db-downgrade:
	uv run --directory backend ibom_ai_agent_stack db downgrade

db-current:
	uv run --directory backend ibom_ai_agent_stack db current

db-history:
	uv run --directory backend ibom_ai_agent_stack db history

# === Server ===
run:
	uv run --directory backend ibom_ai_agent_stack server run --reload

run-prod:
	uv run --directory backend ibom_ai_agent_stack server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend ibom_ai_agent_stack server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend ibom_ai_agent_stack user create-admin

user-create:
	uv run --directory backend ibom_ai_agent_stack user create

user-list:
	uv run --directory backend ibom_ai_agent_stack user list

# === Celery ===
celery-worker:
	uv run --directory backend ibom_ai_agent_stack celery worker

celery-beat:
	uv run --directory backend ibom_ai_agent_stack celery beat

celery-flower:
	uv run --directory backend ibom_ai_agent_stack celery flower
	@echo ""
...
help:
	@echo ""
	@echo "ibom_ai_agent_stack - Available Commands"
	@echo "======================================"
	@echo ""
	...
	@echo ""
	@echo "RAG:"
	@echo "  uv run ibom_ai_agent_stack rag-ingest <path> -c <collection>  Ingest files"
	@echo "  uv run ibom_ai_agent_stack rag-search <query> -c <collection>  Search"
	@echo "  uv run ibom_ai_agent_stack rag-collections                     List collections"
	@echo "  uv run ibom_ai_agent_stack rag-sources                         List sync sources"
	@echo "  uv run ibom_ai_agent_stack rag-source-add                      Add sync source"
	@echo "  uv run ibom_ai_agent_stack rag-source-sync <id>                Trigger sync"
	@echo ""
	@echo "Celery:"
