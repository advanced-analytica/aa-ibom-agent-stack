...
└── commands/             # Click CLI commands (auto-discovered by `ibom_ai_agent_stack cmd …`)
...
The generated project ships a Click CLI exposed as `ibom_ai_agent_stack` (after `make install`):

```bash
ibom_ai_agent_stack server run --reload          # dev server
ibom_ai_agent_stack db upgrade                   # apply migrations
ibom_ai_agent_stack db migrate -m "message"      # create new migration
ibom_ai_agent_stack user create-admin            # interactive admin creation
ibom_ai_agent_stack rag-ingest <path> -c docs    # ingest local files
ibom_ai_agent_stack rag-search "query" -c docs   # semantic search
ibom_ai_agent_stack rag-collections              # list collections
ibom_ai_agent_stack celery worker                # start worker
ibom_ai_agent_stack celery beat                  # start scheduler
```

Run `make help` for a categorized list, or `ibom_ai_agent_stack --help` for full CLI docs.
...
POSTGRES_DB=ibom_ai_agent_stack
GOOGLE_API_KEY=…
```
...
Using **pgvector** as the vector store with **gemini** embeddings.

```bash
# Ingest local files (recursive)
ibom_ai_agent_stack rag-ingest /path/to/docs/ --collection documents --recursive

# Semantic search
ibom_ai_agent_stack rag-search "your query" --collection documents
```
