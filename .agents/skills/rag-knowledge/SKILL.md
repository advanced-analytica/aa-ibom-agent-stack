---
name: rag-knowledge
description: Work with the RAG knowledge base — ingest documents, run semantic search, manage collections, or add a sync source/connector (Google Drive, S3). Use when populating or debugging the knowledge base, tuning retrieval, or adding a new document source. This project uses pgvector + gemini embeddings.
---
...
```bash
uv run ibom_ai_agent_stack cmd rag-ingest ./docs/ --collection docs --recursive   # ingest files/folder
uv run ibom_ai_agent_stack cmd rag-search "your question" --collection docs        # semantic search
uv run ibom_ai_agent_stack cmd rag-collections                                     # list collections
uv run ibom_ai_agent_stack cmd rag-stats                                           # chunk/vector counts
uv run ibom_ai_agent_stack cmd rag-drop <collection> --yes                         # delete a collection
```
...
```bash
uv run ibom_ai_agent_stack cmd rag-sources                  # list
uv run ibom_ai_agent_stack cmd rag-source-add               # add (interactive)
uv run ibom_ai_agent_stack cmd rag-source-sync --all        # trigger a sync
```
...
- Embedding provider and dimensions are fixed per project (gemini) — don't mix embeddings across a collection; re-ingest if you change them.
